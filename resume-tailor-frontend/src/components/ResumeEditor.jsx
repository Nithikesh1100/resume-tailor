...
export default function ResumeEditor() {
  ...
  const downloadPdf = async () => {
    ...
    try {
      const sanitizedLatex = latexCode
        // Remove or replace problematic characters
        .replace(/\r\n/g, "\n") // Normalize line endings
        .replace(/\r/g, "\n") // Convert remaining carriage returns
        .replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g, "") // Remove control characters except \n and \t
        // Ensure proper LaTeX line breaks
        .replace(/\n\s*\n/g, "\n\n") // Normalize multiple newlines
        .trim()

      console.log("[v0] Sanitized LaTeX content length:", sanitizedLatex.length)

      // Call backend API to compile LaTeX to PDF
      const pdfBlob = await compileToPdf(sanitizedLatex)
      ...
      let errorMessage = "Failed to compile PDF. Please check your LaTeX syntax or try again later."

      if (err.message.includes("controlLF") || err.message.includes("font")) {
        errorMessage =
          "PDF compilation failed due to font encoding issues. This usually happens with special characters. Please check your LaTeX content for unusual characters."
      } else if (err.message.includes("500")) {
        errorMessage = "Server error during PDF compilation. Please try again in a moment."
      } else if (err.message.includes("Failed to fetch")) {
        errorMessage = "Unable to connect to the PDF service. Please check your internet connection."
      }
